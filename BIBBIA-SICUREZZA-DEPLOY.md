...
|----------|--------|--------|------|------|
| `/api/contact` | 3 req/IP | 15 min | In-memory | `lib/rate-limiter.ts` |
| `/api/chat` | 10 msg/IP/min + 50/session | 1 min / session | In-memory | `lib/security/rate-limiter-chat.ts` |

**Problema**: Rate limiter in-memory non persiste tra invocazioni serverless. Su Vercel, ogni function invocation può avere un processo diverso. Il rate limiter funziona solo se la stessa istanza serve più richieste (warm function).

**Verdetto**: Funzionale per MVP, inadeguato per produzione seria. Migrare a Vercel KV o Upstash Redis.
...
| CVE scan automatico | ❌ NO | Nessun `npm audit` in CI |
| Aggiornamenti controllati | ⚠️ Manuale | Nessun Dependabot configurato |
...
| NEXT_PUBLIC_ prefix corretto | ✅ OK | Solo SITE_URL è public |
...
**Stato attuale**: In-memory (non persiste tra serverless invocations)

**Target**: Vercel KV (Upstash Redis) per rate limiting distribuito

**STOP richiesto**: Creazione Vercel KV store richiede azione del proprietario su dashboard Vercel.
